...
function SupportPage({ email }) {
  ...
  const sendSupportEmail = () => {
    setLoading(true);
    axios.post(
      'https://api.hbcreations.io/api/sendSupportEmail',
      { to: email, message }
    ).then(res => {
      if (res.status === 201) {
        setLoading(false);
        ...
        <div style={{ display: 'flex', flexDirection: 'row'}}>
          ...
          <Box component="form" sx={{  display: 'flex', flexDirection: 'row' }} noValidate autoComplete="off">
            <Card raised sx={{ bgcolor: '#fffff', paddingY: '4vh', paddingX: '2vw', width: '30vw'}}>
              ...
              <p style={{ fontSize: '18px' }}>We apologize for any inconveniences you may be facing. Let us know whats going on and we will respond to your message as soon as we can.</p>
              <TextField
                  required
                  fullWidth 
                  multiline
